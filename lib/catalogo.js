...
// hoy guarda el CODIGO, no el slug - hay que resolverlo para armar el
// link. Devuelve { proyecto, slug } o null si no se encuentra.
export function getIndividuoPorCodigo(codigo) {
  const ind = leerTodos().find((i) => i.codigo === codigo && i.publicado);
  return ind ? { proyecto: ind.proyecto, slug: ind.slug } : null;
}
